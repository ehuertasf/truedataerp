...
Ext.onReady(function(){
	...
	var selectedRow;
	var st1;
	var txtMoneda,txtPrefijo,chkEstado;
	...
	var txtBuscado = new Ext.form.TextField({
		emptyText 	:'Buscar Moneda...',
        id			:'v1',
        width		:300,
        allowBlank	:false,
		fieldLabel	:'Buscar Moneda'
    });
	...
	var btn_xls = new Ext.Button({
		...
		txtMoneda = new Ext.form.TextField({
			emptyText 	:'Ingrese Moneda...',
        	id			:'txt1',
        	name		:'ntxt1',
        	width		:330,
        	labelAlign	:'right',
        ...
        txtPrefijo = new Ext.form.TextField({
                            ...
                            fn: function(btn,text){
                                if(btn=='yes'){
                                    formulario.form.submit({
                                        url: 'php_admin/p_admin_moneda.php',
                                        method: 'POST',
                                        params: {
                                            action: 'nuevo',
        ...
        function exportar_xls(){
            ...
            var idusuario	=Ext.getDom('idusuario').value;
            document.location='php_admin/exp_moneda.php?usuario='+login+'&idusuario='+idusuario;
        }
        ...
        function deleteRecord(btn) {
            if (btn == 'yes') {
                ...
                var id_moneda	=selectedRow.get(grid.getColumnModel().getDataIndex(0));
        ...
        function muestra_datos(){
                ...
                //alert(m_idservicio);
                var m_moneda      =selectedRow.get(grid.getColumnModel().getDataIndex(1));
                //alert(m_servicio);
                ...
                llama_formulario('MODIFICA MONEDA','modifica');
                //alert("test");
                //Ventana_modal.show();
	...
	var store1= new Ext.data.Store({
	    proxy: new Ext.data.HttpProxy({
	        url: 'php_admin/p_lista_moneda.php?n=1'
	    }),
	    ...
	    remoteSort: false,
	    sortInfo: {field: 'nom_moneda', direction: 'DESC'}
	});
	...
	var grid = new Ext.grid.GridPanel({
	    ...
	    store: store1,
	    columns:[
		        { id:'id_moneda',header: "id_moneda",width: 100 ,sortable: false,groupable: false,dataIndex:'id_moneda',hidden:true},
		        { header: "Moneda",width: 150,sortable: true,dataIndex: 'nom_moneda',align:'left'},
                { header: "Prefijo",width: 600,sortable: true,dataIndex: 'pref_moneda',align:'left'},
                { header: "Estado",width: 600,sortable: true,dataIndex: 'estado',align:'left'}
	    ],
	...
	function Buscar(){
		...
		if(txtBuscado.validate()==true){
	    	store1.proxy= new Ext.data.HttpProxy({url: 'php_admin/p_lista_moneda.php?n=2&buscar='+buscar});
	    	store1.load();
	    	Ext.get("v1").focus();
	...
	function recargar_grid(){
	   	store1.proxy= new Ext.data.HttpProxy({url: 'php_admin/p_lista_moneda.php?n=1'});
	  	store1.load();    		
        //Ext.get("v1").focus();
		//view.startCollapsed=true;
	}
