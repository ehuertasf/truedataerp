...
Ext.onReady(function(){
	...
	var data;
	var selectedRow;
	var store1, store_modelo, store_marca;
	var txtNombre,txtDireccionI,txtDireccionII,txtBorne,txtLongitud,txtLatitud,txtFecInstalacion,txtSerie;
    var xcbo_marca,xcbo_modelo;
	...
	function exportar_xls(){
		...
		var idusuario	=Ext.getDom('idusuario').value;
		document.location='exp_tap.php?usuario='+login+'&idusuario='+idusuario;
	}	
	
	var store1= new Ext.data.GroupingStore({
	    proxy: new Ext.data.HttpProxy({
	        url: 'p_lista_tap.php?n=1'
	    }),
	    ...
	    remoteSort: false,
	    sortInfo: {field: 'nom_tap', direction: 'DESC'}
	});
	...
	var grid = new Ext.grid.GridPanel({
	    ...
	    view: view,
	    columns:[
		        { id:'id_tap',header: "id_tap",width: 100 ,sortable: false,groupable: false,dataIndex:'id_tap',hidden:true},
		        { header: "Nombre Tap",width: 80,sortable: true,dataIndex: 'nom_tap',align:'left'},
                ...
                { header: "Cantidad Borne",width: 80,sortable: true,dataIndex: 'cant_borne',groupable:false},
                ...
                { header: "Fecha Instalacion",width: 200,sortable: true,dataIndex: 'fec_instalacion',groupable:true,align:'left'},
                { header: "Numero Serie",width: 200,sortable: true,dataIndex: 'num_serie',groupable:true,align:'left'},
                { header: "Modelo",width: 200,sortable: true,dataIndex: 'id_modelo',groupable:true,align:'left'},
                { header: "Marca",width: 200,sortable: true,dataIndex: 'id_marca',groupable:true,align:'left'}
	...
	function muestra_datos(){
			...
			var m_borne            =selectedRow.get(grid.getColumnModel().getDataIndex(4));
			...
			var m_serie       =selectedRow.get(grid.getColumnModel().getDataIndex(8));
			var m_modelo        =selectedRow.get(grid.getColumnModel().getDataIndex(9));
            var m_marca        =selectedRow.get(grid.getColumnModel().getDataIndex(10));
		
			
			llama_formulario('MODIFICA TAP','modifica');
			
			txtNombre.value             =m_nombre;
			txtDireccionI.value			=m_direccion;
			txtDireccionII.value		=m_direccion2;
			txtBorne.value              =m_borne;
	...
	function nuevo_producto(){		
		llama_formulario('NUEVO TAP','nuevo');
		Ventana_modal.show();			
	}
	...
	function llama_formulario(titulo,accion){
		...
		txtBorne = new Ext.form.TextField({
			emptyText 	:'Ingrese Borne...',
        	id			:'txt4',
        	name		:'ntxt4',
        	width		:50,
        	allowBlank	:true,
	...
	function Buscar(){
		...
		if(txtBuscado.validate()==true){
	    	store1.proxy= new Ext.data.HttpProxy({url: 'p_lista_tap.php?n=2&buscar='+buscar});
	    	store1.load();
	    	Ext.get("v1").focus();
    ...
    function graba_nuevo(){
    	...
    	var j_direccion1    =Ext.getCmp("txt2").getValue();
    	var j_direccion2	=Ext.getCmp("txt3").getValue();
    	var j_borne         =Ext.getCmp("txt4").getValue();
    	var j_longitud      =Ext.getCmp("txt5").getValue();
       	var j_latitud   	=Ext.getCmp("txt6").getValue();
        var j_fecinstalacion=Ext.getCmp("txt7").getValue();
        var j_serie         =Ext.getCmp("txt8").getValue();
    ...
    function graba_modificacion(){
    	...
    	var id_tap	=selectedRow.get(grid.getColumnModel().getDataIndex(0));
	...
	function recargar_grid(){
	   	store1.proxy= new Ext.data.HttpProxy({url: 'p_lista_tap.php?n=1'});
	  	store1.load();    		
		Ext.get("v1").focus();
		view.startCollapsed=true;			
	}
