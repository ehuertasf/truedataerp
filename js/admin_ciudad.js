...
Ext.onReady(function(){
	...
	var st1,store_cbo;
	var txtDesc;
	var xcbo_departamento,xcbo_estado;
	...
	var txtBuscado = new Ext.form.TextField({
		emptyText 	:'Buscar Ciudad...',
        id			:'v1',
        width		:300,
        allowBlank	:false,
		fieldLabel	:'Buscar Ciudad'
    });
	...
	function exportar_xls(){
		...
		var idusuario	=Ext.getDom('idusuario').value;
		document.location='exp_ciudad.php?usuario='+login+'&idusuario='+idusuario;
	}	
	
	var store1= new Ext.data.GroupingStore({
	    proxy: new Ext.data.HttpProxy({
	        url: 'p_lista_ciudad.php?n=1'
	    }),
	    ...
	    remoteSort: false,
	    sortInfo: {field: 'ciudad', direction: 'DESC'}
	});
	...
	var grid = new Ext.grid.GridPanel({
	    ...
	    view: view,
	    columns:[
		        { id:'idciudad',header: "idciudad",width: 100 ,sortable: false,groupable: false,dataIndex:'idciudad',hidden:true}, 	
		        	{ header: "Ciudad",width: 200,sortable: true,dataIndex: 'ciudad',align:'left'},
		        	{ header: "Departamento",width: 100,sortable: true,dataIndex: 'departamento',groupable:false,hidden:false},
		        	{ header: "iddepartamento",width: 100,sortable: true,dataIndex: 'iddepartamento',groupable:false,hidden:true},
		        	{ header: "Estado",width: 100,sortable: true,dataIndex: 'estado',groupable:false,hidden:false}
	    ],
	...
	function muestra_datos(){
			...
			llama_formulario('MODIFICA CIUDAD','modifica');	    
	...
	function deleteRecord(btn) {
		if (btn == 'yes') {
			...
			var idciudad	=selectedRow.get(fieldName);
			
	        Ext.Ajax.request({
	        	url: 'p_admin_ciudad.php',
	            params: {
	                tarea		:'borrar',
					idciudad	:idciudad,
	                idusuario	:idusuario
	...
	/****************************************************************************************************************/
	/*  VENTANA PARA AGREGAR NUEVA CIUDAD */
	/****************************************************************************************************************/
	...
	function Buscar(){
		...
		if(txtBuscado.validate()==true){
	    	store1.proxy= new Ext.data.HttpProxy({url: 'p_lista_ciudad.php?n=2&buscar='+buscar});
	    	store1.load();
	    	Ext.get("v1").focus();
    ...
    function graba_nuevo(){
    	var j_dpto	=Ext.getCmp("idcbo_dpto").getValue();
        ...
        Ext.Ajax.request({
            ...
            success:function(response,options){
    			...
    			if(respuesta==2){
					...
					Ext.MessageBox.alert('Aviso','Se agrego nueva ciudad');
					recargar_grid();				
    			}
    ...
    function graba_modificacion(){
    	...
    	var j_idciudad	=selectedRow.get(grid.getColumnModel().getDataIndex(0));
	...
	function recargar_grid(){
	   	store1.proxy= new Ext.data.HttpProxy({url: 'p_lista_ciudad.php?n=1'});
	  	store1.load();    		
		//Ext.get("v1").focus();
	}
	...
	var Ventana = new Ext.Window({
		title: 'MODULO -> MANTENIMIENTO DE CIUDADES',
		width: 350,
		autoHeight:450,
		minWidth: 300,
		minHeight: 150,
