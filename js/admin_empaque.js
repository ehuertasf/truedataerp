...
Ext.onReady(function(){
	...
	var txtBuscado = new Ext.form.TextField({
		emptyText 	:'Buscar Empaque...',
        id			:'v1',
        width		:300,
        allowBlank	:false,
		fieldLabel	:'Buscar empaque'
    });
	...
	function exportar_xls(){
		...
		var idusuario	=Ext.getDom('idusuario').value;
		document.location='exp_empaque.php?usuario='+login+'&idusuario='+idusuario;
	}	
	
	var store1= new Ext.data.GroupingStore({
	    proxy: new Ext.data.HttpProxy({
	        url: 'p_lista_empaque.php?n=1'
	    }),
	    ...
	    remoteSort: false,
	    sortInfo: {field: 'desc_empaque', direction: 'DESC'}
	});
	...
	var grid = new Ext.grid.GridPanel({
	    ...
	    store: store1,
	    columns:[
		        { id:'idempaque',header: "idempaque",width: 100 ,sortable: false,groupable: false,dataIndex:'idempaque',hidden:true}, 	
		        	{ header: "Descripci&oacute;n",width: 200,sortable: true,dataIndex: 'desc_empaque',align:'left'},
	...
	function muestra_datos(){
			...
			llama_formulario('MODIFICA EMPAQUE','modifica');	    
	...
	function deleteRecord(btn) {
		if (btn == 'yes') {
			...
			var idempaque	=selectedRow.get(fieldName);
			
	        Ext.Ajax.request({
	        	url: 'p_admin_empaque.php',
	            params: {
	                tarea		:'borrar',
					idempaque	:idempaque,
	                idusuario	:idusuario
	...
	/****************************************************************************************************************/
	/*  VENTANA PARA AGREGAR NUEVO EMPAQUE */
	/****************************************************************************************************************/
	function nuevo_producto(){		
		llama_formulario('NUEVO EMPAQUE','nuevo');	    
		Ventana_modal.show();			
	}
	...
	function Buscar(){
		...
		if(txtBuscado.validate()==true){
	    	store1.proxy= new Ext.data.HttpProxy({url: 'p_lista_empaque.php?n=2&buscar='+buscar});
	    	store1.load();
	    	Ext.get("v1").focus();
    ...
    function graba_modificacion(){
    	...
    	var j_idempaque	=selectedRow.get(grid.getColumnModel().getDataIndex(0));
	...
	function recargar_grid(){
	   	store1.proxy= new Ext.data.HttpProxy({url: 'p_lista_empaque.php?n=1'});
	  	store1.load();    		
		//Ext.get("v1").focus();
	}
	...
	var Ventana = new Ext.Window({
		title: 'MODULO -> MANTENIMIENTO DE EMPAQUES',
		width: 350,
		autoHeight:450,
		minWidth: 300,
		minHeight: 150,
