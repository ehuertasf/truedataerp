...
var store_estado=new Ext.data.Store({			
			proxy		: new Ext.data.HttpProxy({url: "query.php?n=22&idestado=2,3"}),
			reader		: new Ext.data.JsonReader({root: 'estado'}, [
		        {name: 'idestadonotaped', mapping: 'idestadonotaped'},
				{name: 'desc_estadonotaped', mapping: 'desc_estadonotaped'}
			    ]),
			autoLoad	:true
});
...
var cm_bandeja = new Ext.grid.ColumnModel([
		...
		{header: 'Estado',dataIndex: 'desc_estadonotaped',width: 50,align: 'right',
           editor: new Ext.form.ComboBox({
				width		: 200,
				id			: 'idcbo_estado',
				store		: store_estado,
		        displayField:'desc_estadonotaped',
				valueField	: 'idestadonotaped',
		        typeAhead	: true,
		        mode		: 'local',
		        triggerAction: 'all',
				anchor:'95%',
...
var store_bandeja= new Ext.data.GroupingStore({
	proxy		: new Ext.data.HttpProxy({url: 'query.php?n=19&estado=1'}),
...
grid_bandeja.on('afteredit', handleEdit);	
...
function handleEdit(editEvent) {
	...
	function showResultText(btn, text){
		if (btn == 'ok') {
			new_idestado_cc = editEvent.record.data.desc_estadonotaped;
			if (new_idestado_cc != '') {
				Ext.Ajax.request({
					url		: 'query.php',
					params	: {	idnotapedido: editEvent.record.data.idnotapedido,
								obs			: text,
								usuario		: idusuario,
								idestado	: new_idestado_cc,
								n			: 23},
					method: 'GET',
					success: function(result, request){
							var respuesta=result.responseText;
								if (respuesta = 'OK') {
									Ext.Msg.alert('Mensage', 'Estado Actualizado');
									store_bandeja.reload();
									store_bandeja_facturado.reload();
								}
							},
...
var store_estado_facturado=new Ext.data.Store({			
			proxy		: new Ext.data.HttpProxy({url: "query.php?n=22&idestado=5"}),
			reader		: new Ext.data.JsonReader({root: 'estado'}, [
		        {name: 'idestadonotaped', mapping: 'idestadonotaped'},
				{name: 'desc_estadonotaped', mapping: 'desc_estadonotaped'}
			    ]),
...
var cm_bandeja_facturado = new Ext.grid.ColumnModel([
		{header: "Nro.Nota.Ped.",dataIndex: 'idnotapedido',sortable: true,width: 40,align:'center'},
		{header: "Almacen",dataIndex: 'desc_almacen',sortable: true,width: 45,align:'center'},
		{header: "IdCli",dataIndex: 'idcliente',sortable: false,width: 40,align:'center',hidden:true},
		{id:'nombre',header: "Cliente",dataIndex: 'nombre',sortable: true,width: 100,summaryType:'count',
		...
		{header: 'Estado',dataIndex: 'desc_estadonotaped',width: 50,align: 'right',
           editor: new Ext.form.ComboBox({
				width		: 200,
				id			: 'idcbo_estadofacturado',
				store		: store_estado_facturado,
		        displayField:'desc_estadonotaped',
				valueField	: 'idestadonotaped',
		        typeAhead	: true,
		        mode		: 'local',
...
var store_bandeja_facturado= new Ext.data.GroupingStore({
	proxy		: new Ext.data.HttpProxy({url: 'query.php?n=19&estado=4'}),
	reader		: new Ext.data.JsonReader({
		root	: 'bandejapedido',
		fields	: [	{name:'idnotapedido', type:'int'},
					{name:'desc_almacen'},
...
var grid_bandeja_facturado = new Ext.grid.EditorGridPanel({
	store: store_bandeja_facturado,
	cm: cm_bandeja_facturado,
	width:710,
	height:190,
	plugins: summary2,
	view: new Ext.grid.GroupingView({
...
store_bandeja_facturado.load();
//grid_bandeja.on('rowdblclick', gestion);
grid_bandeja_facturado.on('afteredit', handleEdit);	
...
function gestion(grid, rowIndex, e){
	...
	var idpedido= record.data.idnotapedido;

//	window.open(href, windowname, ',type=fullWindow,fullscreen,scrollbars=yes');
	window.open("http://192.168.1.5/apache2-default/automotriz/imprimir_notapedido.php?idpedido="+idpedido,"ventana1" , "width=850,height=650,scrollbars=YES,resizable=YES");
}



function gestion2(grid, rowIndex, e) {
...
var txt_diaspago = new Ext.form.TextField({
	id : 'idtxt_diaspago',
	fieldLabel: 'Plazo en pago en dias',
	disabled: true,
	anchor:'95%'
});
...
var boton_documento = new Ext.Toolbar.Button({
	...
	menu: 	{items: ['<b class="menu-title">Escoja Documento a Generar</b>',
					{	text: 'Factura',
						...
						group : 'radiodocumento',
						checkHandler: onItemCheck
					},'-','<b class="menu-title">Adicional</b>',
...
var factura = new Ext.FormPanel({
        ...
        width: 750,
		frame:true,	
		tbar: [boton_documento,'-','Nro. Documento:',txt_nrodoc
		/*	{text: 'Agregar',
										iconCls:'agregar_item'
						            	//handler : agregarproducto
			},'-',{	text:'Eliminar',
...
*/	
	if (trim(txt_nrodoc.getValue())=='')
		{	//swcabecera=1;
			Ext.MessageBox.alert('Mensaje de validaci&oacute;n','Ingrese Nro. de Documento');
			return;
	}else{
		...
		if (cant_doc==0){
			...
			if (arreglodocumento.indexOf('BD2')!=-1) idtipo_doc=4;
			if (arreglodocumento.indexOf('FD2')!=-1) idtipo_doc=5;
			//alert(idtipo_doc);
...
var	tab_factura = new Ext.TabPanel({
	...
	activeTab:0,
	items:[{	title		: 'Pedidos Pendientes',
				closable	: false,
				autoScroll	: true,
				layout		: 'fit',
				items		: [grid_bandeja]
...
var ventana = new Ext.Window({
	title	:	'Bandeja de Nota de Pedido para Almacen',
	width:950,
	height:450,
	minWidth: 750,
	minHeight: 450,
